// bench_burst.cpp — cpp109 burst 延迟基准（游戏 tick 模型）
//
// 模型: 所有线程共享同一 tick 网格，每帧开头不限速写入 --per-tick 条，
//       其余时间静默；统计每帧 burst span、入队延迟 P50/P99/P99.9/max、
//       >1ms 停顿次数、跨帧溢出次数与帧对齐偏差。
//
// 用法: bench_burst.exe --threads 8 --per-tick 156 --tick-ms 15.625 --seconds 20
...
static void backend_init() {
    ...
    g_logger = cpp109::get_logger("burst");
