...
public:
    ...
    std::shared_ptr<Logger> set_default_logger(std::shared_ptr<Logger> logger){
        std::atomic_store_explicit(&default_logger_, logger, std::memory_order_release);
        return logger;
    }

    std::shared_ptr<Logger> default_logger(){
        auto d = std::atomic_load_explicit(&default_logger_, std::memory_order_acquire);
        if (d) return d;

        std::lock_guard<std::mutex> lock(mutex_);
        d = std::atomic_load_explicit(&default_logger_, std::memory_order_relaxed);
        if (d) return d;
        ...
        std::atomic_store_explicit(&default_logger_, d, std::memory_order_release);
        return d;
    }
    ...
    void add_default_sink(std::shared_ptr<Sink> sink){
        ...
        auto d = std::atomic_load_explicit(&default_logger_, std::memory_order_relaxed);
